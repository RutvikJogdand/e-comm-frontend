...
export default function Cart(props) {
  ...
  const [productsArr, setProductsArr] = useState([...props.productsData])

  useEffect(() => {
    setProductsArr([...props.productsData]);
  }, [props.productsData]);
                      ...
                      <div className="mt-8">
                        <div className="flow-root">
                          <ul role="list" className="-my-6 divide-y divide-gray-200">
                            {productsArr.filter(product => product.quantity).map((product) => (
                              <li key={product.product_id} className="flex py-6">
                                ...
                                <div className="ml-4 flex flex-1 flex-col">
                                  <div>
                                    ...
                                    <button onClick={() => props.decrement(product.product_id)}>-</button>
                                    <span className="mx-2">{props.counters[product.product_id] || 0}</span>
                                    <button onClick={() => props.increment(product.product_id)}>+</button>
                                    <p className="mt-1 text-sm text-gray-500">Total: ${(product.quantity*product.price)/100}</p>
                                  </div>
                                  <div className="flex flex-1 items-end justify-between text-sm">
                                    ...
                                    <div className="flex">
                                      <button
                                        ...
                                        onClick={() => props.remove(product.product_id)}
                                      >
                                        Remove
                                      </button>
                                    </div>
